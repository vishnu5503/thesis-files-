/*
 * hybrid-mpc-dumbbell.cc  — Multi-run version with pcap + seed support
 * Place at:  ~/ns-3.45/scratch/hybrid-mpc-dumbbell.cc
 */
...
int main (int argc, char *argv[])
{
  ...
  NS_LOG_UNCOND("Hybrid MPC seed=" << seed << " " << nBulk << "B+" << nWeb << "W+" << nUdp << "U");
  ...
  if (enablePcap) {
    bottleneck.EnablePcap("mpc-bottleneck", bottleneckDevs.Get(0), true);
    NS_LOG_UNCOND("PCAP → mpc-bottleneck-0-0.pcap");
  }

  const char* home = std::getenv("HOME");
  std::string qPath = outDir.empty()
      ? (home ? std::string(home)+"/mpc-queue-length.csv" : "mpc-queue-length.csv")
  ...
  if (!outDir.empty() && home) {
    std::string src = std::string(home) + "/mpc-hybrid-results.csv";
    std::string dst = outDir + "/mpc-results.csv";
    std::rename(src.c_str(), dst.c_str());
  }

  std::string xmlPath = outDir.empty() ? "mpc-flow-results.xml" : outDir+"/flow-results.xml";
